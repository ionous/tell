...
type tellValue struct {
	entry *tellEntry // values get written into the entry.
}
...
func (val *tellValue) setPendingValue(p pendingValue) {
...
func (val *tellValue) newNum() charm.State {
	num := new(numValue)
...
func (val *tellValue) tryBool(r rune, makeNext func(str string) charm.State) (ret charm.State) {
	var match string
	var res bool
	if r == 't' {
		match, res = "true", true
